...
values ('CWX.TRANSPORTATION', null, 'cwxxxx', 'cwx@mail.ru', 'qwerty', null, '+996502271004', (select TYPE from TYPES_OF_ACCOUNT where type = 'Employer')),
       ('Azidin', 'Amankulov', 'azidinn', 'azidin@mail.ru', 'azidinmanka', null, '+996555555555', (select TYPE from TYPES_OF_ACCOUNT where type = 'Employer'));



insert into contacts_type(PHONE_NUMBER, telegram, email, facebook_link, linkedin_link)
values ('+996555555555', 'CVK.REDAN', 'REDAN.SILA', 'https://sasaeeaessaeea', 'https://xaxaaxaxxaxaxax');

insert into CONTACTS (type, contact_type)
values ( (select TELEGRAM from CONTACTS_TYPE), (select id from CONTACTS_TYPE where TELEGRAM = 'CVK.REDAN'));


insert into VACANCIES(title, salary, job_description, required_min_experience, required_max_experience, date_of_posted, date_of_updated, active, category_id, author_email)
...
insert into RESUMES(title, required_salary, active, author_email, experience_info_id, education_info_id, category_id, contact_id)
